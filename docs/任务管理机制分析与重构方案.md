...
**问题**:
1. 实际运行中 WebSocket 连接不稳定，导致 `isConnected=false`，触发轮询成为实际主方案
2. 同步处理 `processImage` 直接等待结果，异步处理 `processImageAsync` + `pollTaskStatus` 以1秒间隔轮询
3. `useTaskHistory` 依赖 `isConnected` 状态，但 WebSocket 断开时不会自动切换到轮询模式
4. 前端存在两套并行的状态更新机制：WebSocket 监听 + HTTP 轮询，逻辑复杂且可能冲突
...
const unsubscribe = useCallback((taskId: number) => {
  ...
  if (taskSubscribers) {
    taskSubscribers.shift(); // 只移除最早的订阅者
...
**问题**:
- `unsubscribe` 使用 `shift()` 只移除最早订阅者，如果有多个订阅者会漏掉
- `useTaskHistory.ts:123` 的 useEffect 依赖 `handleTaskUpdate` 等回调，这些回调变化会导致重新订阅
- 新任务创建时没有 WebSocket 推送通知，前端需通过轮询或初始加载获取
- 乐观更新与 WebSocket 更新可能产生冲突（时序问题）

#### 问题5：心跳机制验证 ✅

**前端**: `WebSocketContext.tsx:164-168`
```typescript
...
- ✅ WebSocket 基础设施完善（连接管理、心跳、重连）
- ✅ 前端订阅机制基本健全
- ✅ 心跳机制完整实现（前后端都已完成）
- ✅ V2 路由已有 WebSocket 推送代码
- ✅ TaskQueue 与 WebSocket 已集成（generation_v2.py 使用 asyncio.to_thread + 进度推送）
- ✅ 进度推送已实现（0%, 30%, 60%, 90%, 100%）
- ✅ 前端订阅/取消订阅 bug 已修复（shift → delete）
- ✅ Index.tsx 清理函数已保存 + 依赖简化
- ❌ 轮询仍作为降级方案存在
...
**最后更新**: 2026-01-08
**更新内容**:
- 验证并确认 Index.tsx 修复：添加 wsCleanupRef 保存清理函数、简化依赖项
- 验证并确认 generation_v2.py 修复：添加 30%, 60%, 90% 进度推送
- 验证并确认 useTaskHistory 回调已稳定（空依赖数组）
- 更新评估结论，标记已修复问题
